...
public class DisplayDebug extends AbstractDecisionNode {
	...
	private static final String cloud_sdkJsPathTemplate = "com/marketplace/helper/cloud.js";
	private final String clientScript;
	private final String clientScript_Cloud;
	...
	public DisplayDebug(@Assisted Config config, @Assisted Realm realm) throws NodeProcessException {
		...
		this.clientScript_Cloud = readJS_Cloud();
	}
		...
		if(config.display()) {
				try {
					...
					if(config.pretty() && on_prem_flag){
						...
						ScriptTextOutputCallback scb = getScriptedCallback_Cloud(headerKeys, paramKeys, cookies);
						callbacks.add(scb);
					}
	...
	private ScriptTextOutputCallback getScriptedCallback(List<String> headers, List<String> parameters, List<String> cookies) {
		...
		logger.debug("parameters_string:" + parameters_string);
		logger.debug("cookies_string: " + cookies_string);
		logger.debug("headers_string: " + headers_string);
	...
	private ScriptTextOutputCallback getScriptedCallback_Cloud(List<String> headers, List<String> parameters, List<String> cookies) {
		String parameters_string = parameters.toString();
		parameters_string = parameters_string.replace("\"","\\\"");
		...
		String sdkJs = new StrSubstitutor(sdkConfigMap).replace(this.clientScript_Cloud); // This replaces the values in the this.clientScript (The js file) with the java values
		logger.debug("sdkJs: " + sdkJs);
		ScriptTextOutputCallback callback = new ScriptTextOutputCallback(sdkJs);
		return callback;
	}
	...
	private String readJS_Cloud() throws NodeProcessException {
		URL resource = Resources.getResource(cloud_sdkJsPathTemplate);
		try {
			return Resources.toString(resource, Charsets.UTF_8);
		} catch (IOException ex) {
			logger.error(String.format("%s %s: %s", loggerPrefix, ex.getClass().getCanonicalName(),
